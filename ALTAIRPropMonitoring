...
final float   maxCurrent             =    10.0;  // Amperes (per each motor+ESC)
    ...
    if (infoLine != null) {
      ...
      if (infoLine.length() > 50) {
        ...
        if (altairValues.length == 32) {
          ...
          for (int i = 0; i < 3; ++i) accel[i]   = altairValues[i+23];               // in m/s^2
          ...
          accel[2] += cos(radians(pitch))*cos(radians(roll));  accel[2] *= gravAcc;
          accel[1] += cos(radians(pitch))*sin(radians(roll));  accel[1] *= gravAcc;
          accel[0] -= sin(radians(pitch));                     accel[0] *= gravAcc;          
        }
      }
    }
  }
...
void displayPropulsionSystemInfo() {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    int redAlert = 0, redAlertESC = 0, motorIsRed = 0, escIsRed = 0, propIsRed = 0, propIsGreen = 0;
    if (setting[i] > maxSetting || setting[i] < 0.) redAlert = 1;
    fill(color(1.,1.,1.));
    ellipse(18.  + 235.*i + 240.*isRight, 465.  - 30.*isHigh,                 4., 40.);
    fill(color(1.*redAlert,0.7*(1-redAlert),0));
    noStroke();
    ellipse(18.5 + 235.*i + 240.*isRight, 483.5 - 30.*isHigh - fillHeight/2., 3., fillHeight);
    stroke(0);
    drawType("Power",              30. + 235.*i + 240.*isRight, 460. - 30.*isHigh, 1.*redAlert,      0.,                 0.);
    drawType("set at:",            83. + 235.*i + 240.*isRight, 460. - 30.*isHigh, 1.*redAlert,      0.,                 0.);
    textAlign(RIGHT);
    drawType(nf(setting[i],1,1),  186. + 235.*i + 240.*isRight, 460. - 30.*isHigh, 1.*redAlert,      0.6*(1-redAlert),   0.);
    textFont(smallFont);
    ...
    if (rpm[i] > maxRPM || rpm[i] < 0.) { redAlert = 1; motorIsRed = 1; propIsRed = 1; propIsGreen = 0;
    ...
    stroke(0);
    drawType("RPM:",               83. + 235.*i + 240.*isRight, 500. - 30.*isHigh, 1.*redAlert,      0.,                 0.); 
    textAlign(RIGHT);
    drawType(nf(int(rpm[i])),     173. + 235.*i + 240.*isRight, 500. - 30.*isHigh, 1.*redAlert,      0.6*(1-redAlert),   0.);
    textFont(smallFont);
    ...
    if (temp[i]   > maxTemp || temp[i]   < minTemp) { redAlert    = 1; motorIsRed = 1;
                                                      if (isHigh == 1) { upperMotorIsRed = 1; }
                                                      else             { lowerMotorIsRed = 1; }
                                                    } 
    else                                            { redAlert    = 0; }
    if (temp[i+4] > maxTemp || temp[i+4] < minTemp) { redAlertESC = 1; escIsRed   = 1;} else {redAlertESC = 0;}
    fill(color(1.,1.,1.));
    ...
    fill(color(1.*redAlertESC,0.7*(1-redAlertESC),0));
    ellipse(190.5 + 190.*i + 115.*isRight, 358.5              - fillHeightESC/2., 3., fillHeightESC);    
    stroke(0);
    drawType("Temp:",              83. + 235.*i + 240.*isRight, 540. - 30.*isHigh, 1.*redAlert,      0.,                 0.);
    drawType("Temp:",             202. + 190.*i + 115.*isRight, 355.             , 1.*redAlertESC,   0.,                 0.);
    textAlign(RIGHT);
    drawType(nf(int(temp[i])),    173. + 235.*i + 240.*isRight, 540. - 30.*isHigh, 1.*redAlert,      0.6*(1-redAlert),   0.);
    drawType(nf(int(temp[i+4])),  282. + 190.*i + 115.*isRight, 355.             , 1.*redAlertESC,   0.6*(1-redAlertESC),0.);
    textFont(smallFont);
    ...
    if (current[i] > maxCurrent || current[i] < 0.) { redAlert = 1; motorIsRed = 1; escIsRed = 1;
                                                      if (isHigh == 1) { upperMotorIsRed = 1; }
                                                      else             { lowerMotorIsRed = 1; }
                                                    } 
    else                                            { redAlert = 0; }
    ...
    stroke(0);
    drawType("Curr.:",             83. + 235.*i + 240.*isRight, 580. - 30.*isHigh, 1.*redAlert,      0.,                 0.);
    textAlign(RIGHT);
    drawType(nf(int(current[i])), 173. + 235.*i + 240.*isRight, 580. - 30.*isHigh, 1.*redAlert,      0.6*(1-redAlert),   0.);
    textFont(smallFont);
...
*/
  ...
  if (UM7temp > maxTemp || UM7temp < minTemp) redAlert = 1;
  ...
  stroke(0);
  drawType("Orientation sensor", 697., 260., redAlert,  0.,               0.);
  drawType("temp: ",             697., 285., redAlert,  0.,               0.);
  textAlign(RIGHT);
  drawType(nf(int(UM7temp)),     777., 285., redAlert,  0.7*(1-redAlert), 0.);
  textFont(smallFont);
