...
final String  arduinoPortString      = "COM4"; //"tty.usbmodem"; was the string that used to be here
...
void setup()  {
   ...
   if (arduinoPortName.equals("") && !testArduinoUnconnected) {
     ...
     println(Serial.list());
     print("We were looking for a port name containing the string: "); 
     println(arduinoPortString);
     ...
     if (!testArduinoUnconnected) {
       ...
       print("Connected to ALTAIR via serial port: "); 
...
void displayPropulsionSystemInfo() {
   int upperMotorIsRed = 0, lowerMotorIsRed = 0, upperPropIsGreen = 0, 
   lowerPropIsGreen = 0, upperPropIsRed = 0, lowerPropIsRed = 0;
   ...
   for (int i = 0; i < 4; ++i) {
     ...
     drawType("0",                  14. + 235.*i + 240.*isRight, 486. - 30.*isHigh, 0.35, 35, 0.);
     textFont(mainFont);
     textAlign(LEFT);
     ...
     if (rpm[i] > 0.) { 
       ...
       if (isHigh == 1)   { upperPropIsGreen = 1 - upperPropIsRed; }
         ...
         lowerPropIsGreen = 1 - lowerPropIsRed; 
       }
     }
     if (rpm[i] > maxRPM || rpm[i] < 0.) { 
        motorIsRed = 1; propIsRed = 1; 
        propIsGreen = 0;
        if (isHigh == 1)   { 
          upperMotorIsRed = 1; upperPropIsRed = 1; upperPropIsGreen = 0; 
        }
        else { 
          lowerMotorIsRed = 1; lowerPropIsRed = 1; lowerPropIsGreen = 0; 
        }
     ...
     if (temp[i]   > maxTemp || temp[i]   < minTemp) { 
       motorIsRed = 1;
       if (isHigh == 1) { 
         upperMotorIsRed = 1; 
       }
     ...
     if (temp[i+4] > maxTemp || temp[i+4] < minTemp) { 
       escIsRed = 1;
     ...
     if (current[i] > maxCurrent || current[i] < 0.) { 
       motorIsRed = 1; 
       escIsRed = 1;
   ...
   if (upperPropIsGreen > 0 || upperPropIsRed > 0) { 
    rotateY(propRotAng[4]); ++propRotAng[4]; }
   ...
   if (lowerPropIsGreen > 0 || lowerPropIsRed > 0) { 
    rotateY(propRotAng[5]); ++propRotAng[5]; }
...
/*
   ...
   make3DArrow(100.*accel[1]*flipAccelDirection, 10., 
1.*flipAccelDirection, 0.,                    0.                   );

   fill(1, 0.6, 0.6);
   if (accel[0] < 0.) flipAccelDirection = -1; else flipAccelDirection = 1;
   make3DArrow(100.*accel[0]*flipAccelDirection, 10., 0., 
1.*flipAccelDirection, 0.                   );

   fill(0.4, 0.6, 1);
   if (accel[2] < 0.) flipAccelDirection = -1; else flipAccelDirection = 1;
   make3DArrow(100.*accel[2]*flipAccelDirection, 10., 0., 
0.,                    1.*flipAccelDirection);
*/
...
/*
   stroke(0.9, 0.5, 0.5);
   if (accel[0] < 0.) flipAccelDirection = -1; else flipAccelDirection = 1;
   line(xscreenx +       zdirx*0.2 + ydirx*0.2 + xdirx* 0.11, 
xscreeny - 20. + zdiry*0.2 + ydiry*0.2 + xdiry* 0.11,
        xscreenx +       zdirx*0.2 + ydirx*0.2 + xdirx*(0.11 + 
accel[0]*50./xdist), xscreeny - 20. + zdiry*0.2 + ydiry*0.2 + xdiry*(0.11 
+ accel[0]*50./xdist));
   line(xscreenx +       zdirx*0.2 + ydirx*0.2 + xdirx* 0.11, 
xscreeny - 10. + zdiry*0.2 + ydiry*0.2 + xdiry* 0.11,
        xscreenx +       zdirx*0.2 + ydirx*0.2 + xdirx*(0.11 + 
accel[0]*50./xdist), xscreeny - 10. + zdiry*0.2 + ydiry*0.2 + xdiry*(0.11 
+ accel[0]*50./xdist));
   line(xscreenx +       zdirx*0.2 + ydirx*0.2 + xdirx*(0.11 + 
accel[0]*50./xdist), xscreeny - 25. + zdiry*0.2 + ydiry*0.2 + xdiry*(0.11 
+ accel[0]*50./xdist),
        xscreenx +       zdirx*0.2 + ydirx*0.2 + xdirx*(0.11 + 
(accel[0]*50.+flipAccelDirection*15.)/xdist),
        xscreeny - 15. + zdiry*0.2 + ydiry*0.2 + xdiry*(0.11 + 
(accel[0]*50.+flipAccelDirection*15.)/xdist));
   line(xscreenx +       zdirx*0.2 + ydirx*0.2 + xdirx*(0.11 + 
accel[0]*50./xdist), xscreeny -  5. + zdiry*0.2 + ydiry*0.2 + xdiry*(0.11 
+ accel[0]*50./xdist),
        xscreenx +       zdirx*0.2 + ydirx*0.2 + xdirx*(0.11 + 
(accel[0]*50.+flipAccelDirection*15.)/xdist),
        xscreeny - 15. + zdiry*0.2 + ydiry*0.2 + xdiry*(0.11 + 
(accel[0]*50.+flipAccelDirection*15.)/xdist));
   ...
   stroke(0.0, 0.7, 0.0);
   if (accel[1] < 0.) flipAccelDirection = -1; else flipAccelDirection = 1;
   line(yscreenx +       zdirx*0.2 + xdirx*0.2 + ydirx* 0.11, 
yscreeny +       zdiry*0.2 + xdiry*0.2 + ydiry* 0.11,
        yscreenx +       zdirx*0.2 + xdirx*0.2 + ydirx*(0.11 + 
accel[1]*50./ydist), yscreeny +       zdiry*0.2 + xdiry*0.2 + ydiry*(0.11 
+ accel[1]*50./ydist));
   line(yscreenx +       zdirx*0.2 + xdirx*0.2 + ydirx* 0.11, 
yscreeny + 10. + zdiry*0.2 + xdiry*0.2 + ydiry* 0.11,
        yscreenx +       zdirx*0.2 + xdirx*0.2 + ydirx*(0.11 + 
accel[1]*50./ydist), yscreeny + 10. + zdiry*0.2 + xdiry*0.2 + ydiry*(0.11 
+ accel[1]*50./ydist));
   line(yscreenx +       zdirx*0.2 + xdirx*0.2 + ydirx*(0.11 + 
accel[1]*50./ydist), yscreeny -  5. + zdiry*0.2 + xdiry*0.2 + ydiry*(0.11 
+ accel[1]*50./ydist),
        yscreenx +       zdirx*0.2 + xdirx*0.2 + ydirx*(0.11 + 
(accel[1]*50.+flipAccelDirection*15.)/ydist),
        yscreeny +  5. + zdiry*0.2 + xdiry*0.2 + ydiry*(0.11 + 
(accel[1]*50.+flipAccelDirection*15.)/ydist));
   line(yscreenx +       zdirx*0.2 + xdirx*0.2 + ydirx*(0.11 + 
accel[1]*50./ydist), yscreeny + 15. + zdiry*0.2 + xdiry*0.2 + ydiry*(0.11 
+ accel[1]*50./ydist),
        yscreenx +       zdirx*0.2 + xdirx*0.2 + ydirx*(0.11 + 
(accel[1]*50.+flipAccelDirection*15.)/ydist),
        yscreeny +  5. + zdiry*0.2 + xdiry*0.2 + ydiry*(0.11 + 
(accel[1]*50.+flipAccelDirection*15.)/ydist));
   ...
   stroke(0.4, 0.6, 1.0);
   if (accel[2] < 0.) flipAccelDirection = -1; else flipAccelDirection = 1;
   line(zscreenx +       ydirx*0.2 + xdirx*0.2 + zdirx* 0.11, 
zscreeny +       ydiry*0.2 + xdiry*0.2 + zdiry* 0.11,
        zscreenx +       ydirx*0.2 + xdirx*0.2 + zdirx*(0.11 + 
accel[2]*50./zdist), zscreeny +       ydiry*0.2 + xdiry*0.2 + zdiry*(0.11 
+ accel[2]*50./zdist));
   line(zscreenx + 10. + ydirx*0.2 + xdirx*0.2 + zdirx* 0.11, 
zscreeny +       ydiry*0.2 + xdiry*0.2 + zdiry* 0.11,
        zscreenx + 10. + ydirx*0.2 + xdirx*0.2 + zdirx*(0.11 + 
accel[2]*50./zdist), zscreeny +       ydiry*0.2 + xdiry*0.2 + zdiry*(0.11 
+ accel[2]*50./zdist));
   line(zscreenx -  5. + ydirx*0.2 + xdirx*0.2 + zdirx*(0.11 + 
accel[2]*50./zdist), zscreeny +       ydiry*0.2 + xdiry*0.2 + zdiry*(0.11 
+ accel[2]*50./zdist),
        zscreenx +  5. + ydirx*0.2 + xdirx*0.2 + zdirx*(0.11 + 
(accel[2]*50.+flipAccelDirection*15.)/zdist),
        zscreeny +       ydiry*0.2 + xdiry*0.2 + zdiry*(0.11 + 
(accel[2]*50.+flipAccelDirection*15.)/zdist));
   line(zscreenx + 15. + ydirx*0.2 + xdirx*0.2 + zdirx*(0.11 + 
accel[2]*50./zdist), zscreeny +       ydiry*0.2 + xdiry*0.2 + zdiry*(0.11 
+ accel[2]*50./zdist),
        zscreenx +  5. + ydirx*0.2 + xdirx*0.2 + zdirx*(0.11 + 
(accel[2]*50.+flipAccelDirection*15.)/zdist),
...
*/
   ...
   if (UM7temp > maxTemp || UM7temp < minTemp) if (alarmOn[21] == 0) 
     alarmOn[21] = 2;
   else alarmOn[21] = 0;
...
void makeButtons(float x, float y, int servoNum) {
   ...
   if (mouseX > x && mouseX < x+buttonSquareSize &&
     ...
     if (mousePressed && !blockButtons) {
       ...
       print("Sent command to lower control setting # "); 
       println(servoNum);
       blockButtons = true;
       thread("buttonBlock");
     } else {
